// ============================================================
//  VANGUARD MD — commands/clipboard.js
//  Personal text storage & quick copy helper
// ============================================================
...
module.exports = async (ctx) => {
  ...
  if (!action) {
    return reply(
      '*📋 Clipboard Commands*\n' +
      '`clipboard add <text>` - Save text (auto-numbered)\n' +
      '`clipboard del <num>` - Delete specific clip\n' +
      '`clipboard view <num>` - Show clip content (copy-friendly)\n' +
      '`clipboard list` - Show all clips (first 15 chars)\n' +
      '`clipboard pin <num>` - Pin a clip for quick access\n' +
      '`clipboard pv` - View pinned clip\n' +
      '`clipboard clear` - Delete all clips\n' +
      '_Example: `clipboard add Meeting at 5pm`_'
    )
  }
  ...
  if (action === 'list' || action === 'ls') {
    ...
    if (clips.length === 0) {
      return reply('📋 Clipboard is empty.\nAdd something with `clipboard add <text>`')
    }
  ...
  if (action === 'pin') {
    ...
    return reply(`📌 Pinned clip ${num}. Use \`clipboard pv\` to view it anytime.`)
  }

  // PIN VIEW (pv)
  if (action === 'pv' || action === 'pinview') {
    if (!fs.existsSync(PIN_FILE)) {
      return reply('❌ No clip pinned yet.\nPin one with `clipboard pin <num>`')
    }
  ...
  // Unknown action
  return reply('❌ Unknown action. Use `clipboard` alone to see available commands.')
}
